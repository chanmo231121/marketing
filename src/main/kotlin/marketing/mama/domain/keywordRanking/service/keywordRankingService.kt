    ...
    private fun extractData(keyword: String): List<Map<String, Any>> {
        ...
        if (pcResponse != null) {
            try {
                ...
                println("키워드 '$keyword'의 PC 데이터 파싱 중 오류 발생: ${e.message}")
            }
        }
        ...
        // 모바일 데이터 추출
        val mobileResponse = try {
            Jsoup.connect(mobileUrl)
                .headers(headers)
                .timeout(10000)
                .execute()
        } catch (e: Exception) {
        ...
        if (mobileResponse != null) {
            try {
                ...
                println("키워드 '$keyword'의 Mobile 데이터 파싱 중 오류 발생: ${e.message}")
            }
        }
